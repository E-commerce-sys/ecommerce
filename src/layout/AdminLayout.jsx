import { NavLink, Outlet, useNavigate } from "react-router-dom";

import logo from "../assets/icons/logo.svg";
import { useAuth } from "@/context/AuthContext";
import { useUser } from "@/context/ProfileContext";

function formatDisplayName(attributes) {
  if (!attributes) return "";
  const parts = [attributes.firstName, attributes.lastName].filter(Boolean);
  const joined = parts.join(" ").trim();
  return joined || attributes.email || "";
}

function AdminLayout() {
  const navigate = useNavigate();
  const { logout } = useAuth();
  const { user, loading: userLoading } = useUser();

  const attrs = user?.attributes;
  const displayName = formatDisplayName(attrs);
  const email = attrs?.email ?? "";

  async function handleLogout() {
    await logout();
    navigate("/", { replace: true });
  }
  return (
    <div className="flex gap-4 p-3">
      <aside className="flex flex-col justify-between gap-6 py-2 border border-[rgb(var(--color-border))] rounded w-[250px]">
        <div className="flex flex-col gap-10">
          <div className="flex gap-4 p-3 items-center border-b border-[rgb(var(--color-grey))]">
            <img src={logo} alt="" className="w-10 h-10" />
            <div className="flex flex-col gap-1 justify-center">
              <p className="text-[20px] font-bold text-[rgb(var(--color-primary-5))]">
                Exclusive
              </p>
              <p
                className="text-[12px] text-[rgb(var(--color-text-main-2))] cursor-pointer"
                onClick={() => navigate("/")}
              >
                Back to Shopping
              </p>
            </div>
          </div>

          <div className="flex flex-col gap-20 text-[rgb(var(--color-text-main))]">
            <div className="flex flex-col gap-10">
              {/* Dashboard */}
              <NavLink
                to="dashboard"
                className={({ isActive }) =>
                  `px-5 py-1 ${isActive && "bg-[#FBECED] text-[rgb(var(--color-primary-5))] font-medium"}`
                }
              >
                Dashboard
              </NavLink>

              {/* Managment */}
              <div className="flex gap-2 flex-col">
                <p className="text-[rgb(var(--color-text-main-1))] px-2">
                  Managment
                </p>
                <div className="flex flex-col gap-2">
                  <NavLink
                    to="products"
                    className={({ isActive }) =>
                      `px-5 py-1 ${isActive && "bg-[#FBECED] text-[rgb(var(--color-primary-5))] font-medium"}`
                    }
                  >
                    Products
                  </NavLink>
                  <NavLink
                    to="categories"
                    className={({ isActive }) =>
                      `px-5 py-1 ${isActive && "bg-[#FBECED] text-[rgb(var(--color-primary-5))] font-medium"}`
                    }
                  >
                    Categories
                  </NavLink>
                  <NavLink
                    to="orders"
                    className={({ isActive }) =>
                      `px-5 py-1 ${isActive && "bg-[#FBECED] text-[rgb(var(--color-primary-5))] font-medium"}`
                    }
                  >
                    Orders
                  </NavLink>
                  <NavLink
                    to="users"
                    className={({ isActive }) =>
                      `px-5 py-1 ${isActive && "bg-[#FBECED] text-[rgb(var(--color-primary-5))] font-medium"}`
                    }
                  >
                    Users
                  </NavLink>
                  {attrs?.isSuperAdmin === true ? (
                    <NavLink
                      to="staff"
                      className={({ isActive }) =>
                        `px-5 py-1 ${isActive && "bg-[#FBECED] text-[rgb(var(--color-primary-5))] font-medium"}`
                      }
                    >
                      Staff{" "}
                    </NavLink>
                  ) : null}
                </div>
              </div>

              {/* Marketing */}
              <div className="flex gap-2 flex-col">
                <p className="text-[rgb(var(--color-text-main-1))] px-2">
                  Marketing
                </p>
                <div className="flex flex-col gap-2">
                  <NavLink
                    to="coupons"
                    className={({ isActive }) =>
                      `px-5 py-1 ${isActive && "bg-[#FBECED] text-[rgb(var(--color-primary-5))] font-medium"}`
                    }
                  >
                    Coupons
                  </NavLink>
                  <NavLink
                    to="discounts"
                    className={({ isActive }) =>
                      `px-5 py-1 ${isActive && "bg-[#FBECED] text-[rgb(var(--color-primary-5))] font-medium"}`
                    }
                  >
                    Discounts
                  </NavLink>
                </div>
              </div>

              {/* Content */}
              <div className="flex gap-2 flex-col">
                <p className="text-[rgb(var(--color-text-main-1))] px-2">
                  Content
                </p>
                <div className="flex flex-col gap-2">
                  <NavLink
                    to="contacts"
                    className={({ isActive }) =>
                      `px-5 py-1 ${isActive && "bg-[#FBECED] text-[rgb(var(--color-primary-5))] font-medium"}`
                    }
                  >
                    Contacts
                  </NavLink>
                </div>
              </div>

              {/* System */}
              <div className="flex gap-2 flex-col">
                <p className="text-[rgb(var(--color-text-main-1))]  px-2">
                  System
                </p>
                <div className="flex flex-col gap-2">
                  <div className="flex justify-start px-5 py-1">
                    <button
                      type="button"
                      onClick={() => void handleLogout()}
                      className="cursor-pointer"
                    >
                      Log out
                    </button>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="flex flex-col px-2">
          <p className="font-medium">
            {userLoading ? "…" : displayName || "—"}
          </p>
          <p className="text-sm text-[rgb(var(--color-text-main-2))]">
            {userLoading ? "…" : email || "—"}
          </p>
        </div>
      </aside>
      <div className="flex p-6 border border-[rgb(var(--color-border))] rounded w-full">
        <Outlet />
      </div>
    </div>
  );
}

export default AdminLayout;
