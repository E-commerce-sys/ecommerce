...
function PriceFilters() {
  ...
  const isActive = !!filters.priceSort && !!filters.minPrice && !!filters.maxPrice;
  ...
  return (
    <div className="relative inline-block" ref={wrapperRef}>
      ...
      {open && (
        <div className={`absolute top-full mt-2 ${dir === "rtl" ? "right-0" : "left-0"} bg-white border border-[rgb(var(--color-primary-1))] rounded-lg shadow-lg p-4 w-64 z-50`}>
          ...
          <div className="flex gap-2 mb-4">
            <input
              ...
              onChange={(e) => {
                ...
                if (value < 0) { setError("Negative values are not allowed"); return; }
                setMin(value);
                if (max && value >= max) setError("Max should be bigger than Min");
                else setError("");
              }}
          ...
          {error && <p className="text-red-600 text-xs mb-2 font-medium">{error}</p>}
